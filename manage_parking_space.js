document.addEventListener("DOMContentLoaded", () => {
    ...
    const createConfirmationDiv = document.getElementById("createConfirmation");
    const updateConfirmationDiv = document.getElementById("updateConfirmation");
    const ParkInfo = document.getElementById("create-parking-info");
    const updateParkInfo = document.getElementById("update-parking-info");
    ...
    document.getElementById("updateBtn").addEventListener("click", () => {
      ...
      updateParkInfo.innerText = `Space ID: ${spaceId}, Student ID: ${studentId}, Date: ${date}, Time: ${time}`;
      updateConfirmationDiv.style.display = "block";
    });
  
    document.getElementById("confirmUpdateBtn").addEventListener("click", () => {
      const updatedSpaceId = document.getElementById("spaceId").value;
      const updatedStudentId = document.getElementById("studentId").value;
      const updatedDate = document.getElementById("parkingDate").value;
      const updatedTime = document.getElementById("time").value;
  
      $.ajax({
        url: "ajax/update_parking_space.php",
        type: "POST",
        data: {
          spaceId: pendingParking.spaceId,
        ...
        success: function(data) {
          if (data.success == true) {
            ...
            const parkingLi = document.querySelector(`li[data-parking-id="${pendingParking.spaceId}"]`);
            if (parkingLi) {
              parkingLi.innerText = `Space ID: ${updatedSpaceId}, Date: ${updatedDate}, Time: ${updatedTime}, QR Code: ${pendingParking.qrCode}`;
            }
            location.reload();
          } else {
            alert("Error!");
          ...
          updateConfirmationDiv.style.display = "none";
        }
      });
    });
    ...
    function addParkingToList(parking) {
      ...
      li.innerText = `Space ID: ${parking.spaceId}, Student ID: ${parking.studentId}, Date: ${parking.date}, Time: ${parking.time}, QR Code: ${parking.qrCode}`;
    ...
    // Perform cancellation logic such as updating backend
    $.ajax({
      url: "ajax/cancel_parking_space.php",
      type: "POST",
      data: {
        spaceId: parking.spaceId,
      },
